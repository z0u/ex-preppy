"""Catalyst-based main script for training the color model."""
...
from ex_color.catalyst_train import train_color_model_catalyst
...
def prep_data() -> tuple[DataLoader, torch.Tensor]:
    """
    Prepare data for training using Catalyst format.
...
def train(
    ...
    """Train the model with the given dopesheet and variant using Catalyst."""
    ...
    with tqdm(total=dopesheet.as_df()['STEP'].max()) as p:
        ...
        # Train using Catalyst
        train_color_model_catalyst(
            model,
            hsv_loader,
            rgb_tensor,
            dopesheet,
