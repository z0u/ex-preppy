...
def test_training_module_backwards_compatibility(sample_model, sample_dopesheet, sample_objective):
    """Test that TrainingModule works with old regularizers without layer_affinities."""
    regularizers = [
        RegularizerConfig(
            name='test-reg-old',
            compute_loss_term=MockRegularizer(),
            label_affinities=None,
            # No layer_affinities - should default to encoder
        ),
    ]
...
def test_training_module_invalid_layer_names(sample_model, sample_dopesheet, sample_objective):
    """Test that TrainingModule handles invalid layer names gracefully."""
    regularizers = [
        RegularizerConfig(
    ...
    # Should not crash, but should not register any hooks
